...
export default function LiguesEdit({ ligue }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        nom: ligue.nom,
        logo: ligue.logo || '',
        niveau: ligue.niveau,
        nombre_equipes: ligue.nombre_equipes,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/dashboard/ligues/${ligue.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Modifier ${ligue.nom}`} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Modifier la ligue : {ligue.nom}</h1>
                        <p className="text-gray-600 dark:text-gray-400">Modifiez les informations de votre ligue</p>
                    </div>
                    <Link
                        href="/dashboard/ligues"
