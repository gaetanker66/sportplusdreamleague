...
interface Props { saison: Saison; ligues: Ligue[]; equipes: Equipe[] }

export default function SaisonsEdit({ saison, ligues, equipes }: Props) {
    const { data, setData, put, processing, errors, post } = useForm({
        nom: saison.nom,
        date_debut: saison.date_debut?.slice(0, 10),
        date_fin: saison.date_fin ? saison.date_fin.slice(0, 10) : '',
        status: saison.status as 'en cours' | 'terminé',
        ligue_id: saison.ligue_id,
        nombre_equipes: saison.nombre_equipes,
        equipes: (saison.equipes ?? []).map(e => e.id),
    });
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Modifier ${saison.nom}`} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Modifier la saison : {saison.nom}</h1>
                        <p className="text-gray-600 dark:text-gray-400">Mettez à jour les informations de la saison</p>
                    </div>
                    <button
                        type="button"
                        onClick={handleGenerate}
                        disabled={genLoading || (data.equipes.length !== (data.nombre_equipes || 0))}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select
                                        id="ligue_id"
                                        value={data.ligue_id}
                                        onChange={(e) => setData('ligue_id', parseInt(e.target.value))}
                                        className="mt-1 block w-full px-3 py-2 border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                        required
                                    >
                                        {ligues?.map((ligue) => (
