...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Modifier', href: '/dashboard/histoire-etapes/edit' },
];
...
export default function HistoireEtapesEdit({ etape }: { etape?: HistoireEtape }) {
    if (!etape) {
        return (
            <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                        <p className="text-red-800 dark:text-red-200">Étape non trouvée.</p>
                        <Link 
    ...
    const { data, setData, put, processing, errors } = useForm({
        titre: etape.titre,
        date_label: etape.date_label || '',
        date: etape.date ? etape.date.split('T')[0] : '',
    ...
    const [imagePreview, setImagePreview] = useState<string>(etape.image || '');

    useEffect(() => {
        if (data.image && data.image !== imagePreview) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/dashboard/histoire-etapes/${etape.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Modifier ${etape.titre}`} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Modifier l'étape : {etape.titre}</h1>
                        <p className="text-gray-600 dark:text-gray-400">Mettez à jour les informations de l'étape</p>
                    </div>
                    <Link 
                        href="/dashboard/histoire-etapes" 
