...
export default function PostesIndex({ postes = [] as Poste[] }: { postes: Poste[] }) {
    const handleDelete = (id: number) => {
        if (confirm('Supprimer ce poste ?')) router.delete(`/dashboard/postes/${id}`);
    };
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <Link href="/dashboard/postes/create" className="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">+ Nouveau Poste</Link>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
                    {postes.length ? (
                                        ...
                                        <tr key={poste.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div className="flex gap-2">
                                                    <Link href={`/dashboard/postes/${poste.id}`} className="px-2.5 py-1.5 rounded text-white bg-blue-600 hover:bg-blue-700">Voir</Link>
                                                    <Link href={`/dashboard/postes/${poste.id}/edit`} className="px-2.5 py-1.5 rounded text-white bg-blue-600 hover:bg-blue-700">Modifier</Link>
                        ...
                        <div className="text-center py-12">
                            ...
                            <div className="mt-6">
                                <Link href="/dashboard/postes/create" className="inline-flex items-center px-4 py-2 rounded-md text-white bg-blue-600 hover:bg-blue-700">Nouveau Poste</Link>
                            </div>
                        </div>
                    )}
                </div>
