import { useCoupeAvecPouleModeleLogos } from '@/hooks/use-coupe-avec-poule-modele-logos';
import { cn } from '@/lib/utils';

interface CoupeAvecPouleModeleLogoProps {
    modeleId?: number;
    logo?: string | null;
    nom?: string;
    className?: string;
...
/**
 * Composant pour afficher le logo d'un modèle de coupe avec poules avec chargement asynchrone
 */
export default function CoupeAvecPouleModeleLogo({ 
    modeleId, 
    logo, 
    nom, 
    className = '',
...
}: CoupeAvecPouleModeleLogoProps) {
    const { getLogo } = useCoupeAvecPouleModeleLogos(modeleId ? [modeleId] : []);
